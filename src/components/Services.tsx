...
import { School, Utensils, GraduationCap, PartyPopper, Leaf, Clock } from "lucide-react";

const services = [
  {
    title: "School Cafeteria",
    description: "Nutritious meals designed to fuel growing minds. Our school meal programs follow dietary guidelines while keeping lunch exciting.",
    icon: School,
    color: "bg-blue-500",
  },
  {
    title: "College Dining",
    description: "Modern dining options that meet the diverse tastes and schedules of college students. Meal plans available for all budgets.",
    icon: GraduationCap,
    color: "bg-brand-green",
  },
  {
    title: "Event Catering",
    description: "From graduation ceremonies to conferences, we provide memorable food experiences for all types of academic and social events.",
    icon: PartyPopper,
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {services.map((service, index) => (
            <Card key={index} className="border-none shadow-lg card-hover">
              <CardHeader>
                <div className={`p-3 rounded-lg w-14 h-14 flex items-center justify-center ${service.color} text-white mb-4`}>
              ...
              <CardFooter>
                <Button variant="outline" className="w-full border-brand-dark text-brand-dark hover:bg-brand-dark/10">
                  Learn More
                </Button>
              </CardFooter>
            </Card>
              ...
              <div className="space-y-6">
                {features.map((feature, index) => (
                  <div key={index} className="flex gap-4">
                    ...
                    <div>
                      <h4 className="font-semibold text-brand-dark">{feature.title}</h4>
                      <p className="text-brand-dark/60">{feature.description}</p>
                    </div>
                  </div>
                ))}
              </div>
              <Button className="mt-8 bg-brand-dark hover:bg-brand-dark/90">
                Get a Custom Quote
              </Button>
            </div>
